...
const Game = (() => {
  ...
  function _updateAimKeys(dt) {
    ...
    // Map mouse screen position directly to world target:
    // X: left→right = -22→+22, Y: top→bottom = far→near
    st.targetPoint.x = THREE.MathUtils.clamp(mouse.ndc.x * 24, -22, 22);
    st.targetPoint.z = THREE.MathUtils.clamp(
      THREE.MathUtils.mapLinear(mouse.ndc.y, 0.9, -0.9, 12, 72), 8, 78
    );
    ...
    st.barrelGroup.rotation.y = THREE.MathUtils.clamp(rawYaw, st.minYaw, st.maxYaw);

    const hDist = Math.sqrt(
      Math.pow(st.targetPoint.x - st.group.position.x, 2) +
      Math.pow(st.targetPoint.z - st.group.position.z, 2)
    );
    st.barrelGroup.rotation.x = THREE.MathUtils.clamp(-0.04 - hDist * 0.01, -0.55, -0.04)
                               + (st.recoilX || 0);
  ...
  function _doFire(st, overrideTarget) {
    ...
    const vel    = computeLaunchVelocity(muzzleWorld, target, 22);
