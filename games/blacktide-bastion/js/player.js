...
const Player = (() => {
  ...
  function init(scene) {
    ...
    // Red waist sash
    const sashMat = new THREE.MeshStandardMaterial({ color: 0x8a1818, roughness: 0.85 });
    const sash = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.14, 0.36), sashMat);
    sash.position.y = 0.62;
    mesh.add(sash);

    // Arms — pivot at the shoulders so they swing while running
    const armMat = new THREE.MeshStandardMaterial({ color: 0x14224a });
    const handMat = new THREE.MeshStandardMaterial({ color: 0xd4a07a });
    _arms = [];
    [-0.36, 0.36].forEach(ax => {
      const arm = new THREE.Group();
      arm.position.set(ax, 1.45, 0);
      const sleeve = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.52, 0.18), armMat);
      sleeve.position.y = -0.26;
      arm.add(sleeve);
      const hand = new THREE.Mesh(new THREE.SphereGeometry(0.09, 6, 6), handMat);
    ...
    const bootMat = new THREE.MeshStandardMaterial({ color: 0x241405, roughness: 0.7 });
    [-0.14, 0.14].forEach(lx => {
      const leg = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.36, 0.22), legMat);
      leg.position.set(lx, 0.58, 0);
      mesh.add(leg);
      const boot = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.26, 0.30), bootMat);
      boot.position.set(lx, 0.26, 0.03);
      mesh.add(boot);
    });
  ...
  function update(dt, inputX) {
    ...
    // Animate legs, arms and body bob when moving
    if (state.moving) {
      const bob = Math.sin(state.runTime * 8) * 0.04;
      mesh.position.y = bob;
      bodyMesh.rotation.z = Math.sin(state.runTime * 8) * 0.06;
      _arms[0].rotation.x =  Math.sin(state.runTime * 8) * 0.7;
      _arms[1].rotation.x = -Math.sin(state.runTime * 8) * 0.7;
    } else {
      mesh.position.y = 0;
      bodyMesh.rotation.z *= 0.85;
      _arms[0].rotation.x *= 0.85;
      _arms[1].rotation.x *= 0.85;
    }
